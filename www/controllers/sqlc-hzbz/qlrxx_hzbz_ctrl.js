angular.module('qlrxxHzbzCtrl', []).controller('qlrxxHzbzCtrl', ["$scope", "ionicToast", "$stateParams", "$state", "$ionicHistory", "$wysqService", "$dictUtilsService", "$rootScope", "$menuService",
	function($scope, ionicToast, $stateParams, $state, $ionicHistory, $wysqService, $dictUtilsService, $rootScope, $menuService) {
		...
		//编辑权利人初始化
		function initEdit() {
			$wysqService.getywrByywrId({//因为category ==1 ，目前暂时用义务人接口获取的数据
				id: $stateParams.id
			}).then(function(res) {
				if(res.success) {
					$scope.qlr = res.data;
		...
		function verifyYwData() {
			...
			if($scope.qlr.ywrmc == undefined || $scope.qlr.ywrmc === null || $scope.qlr.ywrmc === "" || $dictUtilsService.hasNum($scope.qlr.ywrmc)) {
