angular.module('ywrxxBgzyCtrl', []).controller('ywrxxBgzyCtrl', ["$scope", "ionicToast", "$stateParams", "$state", "$ionicHistory", "$wysqService", "$dictUtilsService", "$rootScope", "$menuService",
	function($scope, ionicToast, $stateParams, $state, $ionicHistory, $wysqService, $dictUtilsService, $rootScope, $menuService) {
		...
		$scope.addYwr = function() {
			if($scope.category == '1-1') {
				$scope.ywr.ywh = $wysqService.djsqItemData.ywh[0];
			} else if($scope.category == '2-1') {
