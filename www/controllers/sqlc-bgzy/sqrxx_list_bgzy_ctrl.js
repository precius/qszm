angular.module('sqrxxListBgzyCtrl', []).controller('sqrxxListBgzyCtrl', ["$scope", "ionicToast", "$state",
	...
	function($scope, ionicToast, $state, $ionicHistory, $wysqService, $ionicPopup, $ionicLoading, $dictUtilsService,
					...
					if(res.success) {
						...
						if($scope.sqrList != null && $scope.sqrList.length > 0) {
							...
							for(var i = 0; i < $scope.sqrList.length; i++) {
								...
								if(person.category == "0") { //变更权利人
									$scope.bgqlrList[$scope.bgqlrList.length] = person;
								} else if(person.category == "1") { //变更义务人
									$scope.bgywrList[$scope.bgywrList.length] = person;
								} 

							};
							
						}
						$scope.sqrList = [].concat($scope.qlxx.children[1].qlrs ,$scope.qlxx.children[1].ywrs);
						if($scope.sqrList != null && $scope.sqrList.length > 0) {
							$scope.zyqlrList = [];
							$scope.zyywrList = [];
							for(var i = 0; i < $scope.sqrList.length; i++) {
		...
		//跳转到编辑
		$scope.editPersonInfo1 = function(item) {
			$state.go('qlrxxBgzy', {
				'id': item.id,
				'category': '1-0',
				'action': '编辑'
		...
		$scope.editPersonInfo2 = function(item) {
			$state.go('qlrxxBgzy', {
				'id': item.id,
				'category': '2-0',
				'action': '编辑'
		...
		$scope.nextStep = function() {
			if($scope.bgqlrList == null || $scope.bgqlrList.length == 0) {
				$scope.showAlert("缺少变更权利人");
			}  else if($scope.zyqlrList == null || $scope.zyqlrList.length == 0) {
