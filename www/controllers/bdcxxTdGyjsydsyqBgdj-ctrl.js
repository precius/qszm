//不动产信息页面控制器（土地_国有建设用地使用权_变更登记）
angular.module('bdcxxTdGyjsydsyqBgdjCtrl', []).controller('bdcxxTdGyjsydsyqBgdjCtrl', ["$scope", "ionicToast", "$stateParams", "$state", "$ionicHistory", "$ionicActionSheet", "$wysqService", "$dictUtilsService",
	function($scope, ionicToast, $stateParams, $state, $ionicHistory, $ionicActionSheet, $wysqService, $dictUtilsService) {
					...
					.then(function(res) {
						if(res.success) {
							...
							if($scope.qlxx.qlxxEx !== null) {
								/*$scope.bdcxx.tdsyqmj = $scope.qlxx.qlxxEx.tdsyqmj;//土地使用权面积
								$scope.bdcxx.tdyt = $scope.qlxx.qlxxEx.tdyt;//土地用途
								...
								$scope.bdcxx.qlxxEx.tdyt = $scope.qlxx.qlxxEx.tdyt; //土地用途
		...
		$scope.addbdcxx = function(isApply) {
			...
			if($scope.verifyDataBgdj()) { //变更登记数据验证
