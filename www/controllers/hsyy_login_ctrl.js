angular.module('hsyyLoginCtrl', ['ionic']).controller('hsyyLoginCtrl', ["$scope", "ionicToast", "$state", "$loginService", "$ionicPopup", "$ionicLoading", "$meService", "$ionicHistory", "$dictUtilsService", "$rootScope",
			...
			if($scope.user.loginName === null || $scope.user.loginName === "") {
				...
				$loginService.login({
					...
					.then(function(res) {
						...
						$scope.showAlert('登录失败' + res.message);
					});
			}
		};
