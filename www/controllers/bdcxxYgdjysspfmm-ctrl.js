//不动产信息(预告登记-预售商品房买卖预告登记)页面控制器
angular.module('bdcxxYgdjysspfmmCtrl', []).controller('bdcxxYgdjysspfmmCtrl', ["$scope", "ionicToast", "$stateParams", "$state", "$ionicHistory", "$wysqService", "$dictUtilsService",
	function($scope, ionicToast, $stateParams, $state, $ionicHistory, $wysqService, $dictUtilsService) {
		...
		$scope.ygdjzl = $scope.ygdjzlData[0];
		...
		$scope.addbdcxx = function(isApply) {
			...
			$scope.bdcxx.ygdjzl = $scope.ygdjzl.value;
