angular.module('qlrxxYgdyCtrl', []).controller('qlrxxYgdyCtrl', ["$scope", "ionicToast", "$stateParams", "$state", "$ionicHistory", "$wysqService", "$dictUtilsService", "$rootScope", "$menuService",
	function($scope, ionicToast, $stateParams, $state, $ionicHistory, $wysqService, $dictUtilsService, $rootScope, $menuService) {
		...
		$scope.addQlr = function() {
			if($scope.category == '7'||$scope.category == '8') {
				$scope.qlr.ywh = $wysqService.djsqItemData.ywh[0];
			} else if($scope.category == '9'||$scope.category == '10'||$scope.category == '11') {
			...
			if(verifyYwData() && isNotDuplicate() && checkOwnedRegular()) {
				...
				$wysqService.addqlr($scope.qlr).then(function(res) {
					if(res.success) {
						if($scope.category == '7'){//如果是第一部分流程（预告）的权利人添加，那就再当做第二部分流程（抵押）的预告抵押人再添加一次
							$scope.qlr.ywh = $wysqService.djsqItemData.ywh[1];
							$scope.qlr.ywrmc = $scope.qlr.qlrmc;
							$scope.qlr.qlrmc = null;
							$scope.qlr.category = '10';
							$scope.addYwr();
						}else{
							$ionicHistory.goBack();
						}
		...
		$scope.updateYwr = function(){
			$wysqService.updateYwr($scope.qlr).then(function(res) {
				if(res.success) {
					$ionicHistory.goBack();
				} else {
					showAlert(res.message);
