//合并流程 预告+抵押 不动产信息页面控制器
angular.module('bdcxxYgdyCtrl', []).controller('bdcxxYgdyCtrl', ["$scope", "ionicToast", "$stateParams", "$state", "$filter", "$ionicHistory", "$ionicActionSheet", "$wysqService", "$dictUtilsService", "$menuService", "$cordovaBarcodeScanner","$rootScope",
	function($scope, ionicToast, $stateParams, $state, $filter, $ionicHistory, $ionicActionSheet, $wysqService, $dictUtilsService, $menuService, $cordovaBarcodeScanner,$rootScope) {
		...
		$scope.qlxxEx.dyfs = $scope.mode.value;
			...
			$wysqService.queryApplyByYwh({
				...
				if(res.success) {
					...
					if($scope.qlxx.children[0].qlxxEx !== null) {
						...
						if($scope.qlxxEx.dyfs != null) {
							if($scope.qlxxEx.dyfs == 1) {
								$scope.isFwybdyscdj = true;
								$scope.isFwzgedyqscdj = false;
							}
							if($scope.qlxxEx.dyfs == 2) {
								$scope.isFwybdyscdj = false;
								$scope.isFwzgedyqscdj = true;
							}
							for(var i = 0; i < $scope.dyfsData.length; i++) {
								if($scope.dyfsData[i].value == $scope.qlxxEx.dyfs) {
									$scope.mode = $scope.dyfsData[i];
								}
							}
						}
						$scope.qlxxEx.zwlxqssj = new Date($scope.qlxxEx.zwlxqssj);
						$scope.qlxxEx.zwlxjssj = new Date($scope.qlxxEx.zwlxjssj);
		...
		//			$scope.qlxxEx.dyfs = $scope.dyfsData[1].value;
		//			$scope.qlxxEx.dyfsLabel = $scope.dyfsData[1].description;
		//			$scope.dyfs = $scope.dyfsData[1];
		//		} else if($scope.isFwybdyscdj) {
		//			$scope.qlxxEx.dyfs = $scope.dyfsData[0].value;
		//			$scope.qlxxEx.dyfsLabel = $scope.dyfsData[0].description;
		...
		//					$scope.qlxxEx.dyfs = dyfsTemp.description;
		//				}
		//			}
		//		}
		...
		$scope.setDatePlaceHolder = function() {
			...
			if($scope.qlxxEx.zwlxqssj != null || $scope.qlxxEx.zwlxqssj != '' || $scope.qlxxEx.zwlxqssj != "") {
				$(dateinput).addClass('full');
			}
			if($scope.qlxxEx.zwlxjssj != null || $scope.qlxxEx.zwlxjssj != '' || $scope.qlxxEx.zwlxjssj != "") {
		...
		$scope.verifyDataNormal = function() {
			...
			} else if($scope.qlxxEx.dyfs == undefined || $scope.qlxxEx.dyfs === "") {
				$scope.showAlert("请输入抵押方式");
			} else if($scope.qlxxEx.bdbzqse == undefined || $scope.qlxxEx.bdbzqse === null || $scope.qlxxEx.bdbzqse === "" || !$dictUtilsService.number($scope.qlxxEx.bdbzqse)) {
				$scope.showAlert("请输入正确的被担保债权数额");
			} else if($scope.qlxxEx.zwlxqssj == undefined || $scope.qlxxEx.zwlxqssj === null || $scope.qlxxEx.zwlxqssj === "") {
				$scope.showAlert("请选择债务履行起始时间");
			} else if($scope.qlxxEx.zwlxjssj == undefined || $scope.qlxxEx.zwlxjssj === null || $scope.qlxxEx.zwlxjssj === "") {
		...
		$scope.compareStartAndEndDate = function() {
			//$scope.qlxxEx.zwlxqssj 和 $scope.qlxxEx.zwlxjssj  做比较
			var startTime = $scope.qlxxEx.zwlxqssj.getTime();
			var endTime = $scope.qlxxEx.zwlxjssj.getTime();
