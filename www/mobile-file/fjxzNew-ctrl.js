angular.module('fjxzCtrl', []).controller('fjxzCtrl', ["$scope", "ionicToast", "$state", "$ionicHistory", "$wysqService", "$stateParams", "$ionicPopup", "$ionicLoading", "$dictUtilsService", "$fjxzUtilsService",
	function($scope, ionicToast, $state, $ionicHistory, $wysqService, $stateParams, $ionicPopup, $ionicLoading, $dictUtilsService, $fjxzUtilsService) {
		...
		if($stateParams.subFlowcode) {
			...
			var subFlowcode = Request.subCode;
		}
		//根据子流程代码获取附件类型列表
